...
class Classificator:
	...
	def __ParseOperation(self, line: ExecutableLine) -> OperationData:
		...
		if len(Parts) == 2:
			if Operator not in (">", "-"):
				raise ClassificatorExceptions.ScriptRuntimeError(line, f"Unknown operator: \"{Operator}\".")
			
			if Operator == ">":
				raise ClassificatorExceptions.ScriptRuntimeError(line, "Renaming operator requires value.")
			
			return OperationData(target = Parts[0], operator = "-")
		
		if len(Parts) == 3:
			if Operator == "-":
				raise ClassificatorExceptions.ScriptRuntimeError(line, "Deleting operator is unary.")
	...
	def __ReadScriptFile(self, script_file: str | PathLike[str], include: bool = True) -> list[ExecutableLine]:
		...
		for Index in range(len(ScriptLines)):
			...
			if Line.startswith(f"@{Directives.INCLUDE.name}"):
				if not include: raise ClassificatorExceptions.IncludeDirectiveDenied(ScriptPath, Index + 1)
		...
		for Line in script_lines:
			...
			if Line.value.startswith(f"@{Directives.DROP.name}"):
				...
				for Value in Values:
					match Value:
						...
						case _: raise ClassificatorExceptions.ScriptRuntimeError(Line, f"Unknown drop value: \"{Value}\".")
			...
			if Line.value.startswith(f"@{Directives.FORMAT.name}"):
				...
				if FirstValue in ("low", "up"):
					...
					raise ClassificatorExceptions.ScriptRuntimeError(Line, f"Unknown format value: \"{Format}\".")
			...
			if Line.value.startswith(f"@{Directives.TYPE.name}"):
				...
				try:
					...
					raise ClassificatorExceptions.ScriptRuntimeError(Line, f"Unknown type: \"{FirstValue}\".")
			...
			if Line.value.startswith("*"):
				...
				if Operation and LineOperation.operator:
					raise ClassificatorExceptions.ScriptRuntimeError(Line, f"Using operator with \"@{Directives.OPERATION.name}\" directive is denied.")
