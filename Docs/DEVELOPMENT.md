...
Ваша задача заключается в том, чтобы все свойства класса возвращали описываемые ими значения, а также вы должны предоставить рабочие методы `amend`, `parse` и, опционально, такие методы, как `collect`, `image`, и `repair`.
...
def collect(self, period: int | None = None, filters: str | None = None, pages: int | None = None) -> list[str]:
		...
		Собирает список тайтлов по заданным параметрам.
			...
			filters – строка, описывающая параметры фильтрации запроса;\n
...
Filename = Downloader(self.__SystemObjects).temp_image("https://link_to_image")
...
Доступ к настройкам парсера рекомендуется осуществлять через объектное представления `ParserSettings`, однако классический режим также доступен.
...
| 6         | Парсер должен опираться на предоставляемые Melon методы, порты и имплементации.                                                                  |
