...
> [!WARNING]  
> На Windows используйте `.venv\Scripts\activate` вместо `source .venv/bin/activate`. Кроме того, некоторые функции, например использование алиаса `melon` вместо `python main.py`, не поддерживаются в связи с ограничениями системы.
