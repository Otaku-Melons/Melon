...
1. Для установки необходимо наличие пакетов [Git](https://git-scm.com/) и [Python](https://www.python.org/) версии **3.12** или новее на вашем устройстве.
```Bash
git -v
python3 -V
```
2. Клонируйте репозиторий Melon и перейдите в его каталог.
```Bash
git clone https://github.com/otaku-melons/Melon
cd Melon
```
...
pip install .
```
4. После этого вы сможете использовать Melon внутри виртуального окружения, не захламляющего вашу систему пакетами. Для ознакомления с доступными командами выполните:
```Bash
melon help
```
