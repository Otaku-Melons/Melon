...
# Консольные команды
```
collect [PARSER] [FLAGS] [KEYS]
```
Собирает из каталога источника коллекцию алиасов тайтлов по заданным криетриям и помещает её в файл _Collection.txt_, в домашнем каталоге парсера.
...
* _**-f**_ – включает перезапись уже существующей коллекции;
* _**-sort**_ – включает сортировку алиасов в алфавитном порядке.

**Список специфических ключей:**
* _**--filters**_ – указывает параметры запроса каталога, использующиеся для фильтрации нужного контента;
* _**--pages**_ – задаёт количество страниц каталога, с которых нужно получить контент.
___
```
get [URL] [PARSER] [KEYS]
```
